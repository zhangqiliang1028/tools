...
class CircleDetectTool :public ToolBase
{
private:
	...
	int pointsDetection(Mat src,scanArea area,vector<vector<Point>> scanPoints, int edgeThreshold, int selectionMethod, int colorChange, int blurLenth,int blurWidth,int depthThreshold,vector<Point2f> &points );
	int circleDetect(Mat src,scanArea area, int selectPointCount, int blurWidth,int blurLength,int edgeThreshold, int selectionMethod,int colorChange,int depthThreshold,Circle &circle);
	int move_circle(Point2f center, float center_angle, Point2f tempDatum_point,Point2f testDatum_point,scanArea &detectArea);
	bool fittingCircle(std::vector<cv::Point2f>& pts, cv::Point2f& center, float& radius);
public:

	CircleDetectTool(){};
	virtual~CircleDetectTool() {};
