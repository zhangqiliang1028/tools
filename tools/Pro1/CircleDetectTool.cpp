...
bool CircleDetectTool::fittingCircle(std::vector<cv::Point>& pts, cv::Point2f& center, float& radius)
{
	center = cv::Point(0, 0);
	radius = 0.0;
...
int CircleDetectTool::circleDetect(Mat src,scanArea area, int selectPointCount, int blurWidth,int blurLength,int edgeThreshold, int selectionMethod,int colorChange,int depthThreshold,Circle &circle){
	...
	/*for(int i=0;i<points.size();i++){
		cv::circle(image,points[i],2,Scalar(0),-1);	
	}*/
	...
	vector<Point> points_;                              //排除1/10最远点和1/10最近点
	...
	int hit=0;
	for(int i=0;i<points.size();i++){              
		int x_=abs(points[i].x-x0);
		int y_=abs(points[i].y-y0);
		float min=((x_-1)*(x_-1)+(y_-1)*(y_-1));
		float max=((x_+1)*(x_+1)+(y_+1)*(y_+1));
